...
int32_t RtmpPushCamera()
{
	...
	do
	{
		...
		//输出编码
		//推送rtmp流到服务器  
		const char* outFilename = R"(rtmp://127.0.0.1:1935/live/camera)";
		const char* ofmtName = "flv";//输出格式;
		...
		for (int i = 0; i < ifmtCtx->nb_streams; ++i)
		{
			...
			{
				// 调整输出流的一些属性  
				outStream->time_base.num = 1; //inStream->time_base; outStream->time_base必须设置
				...
				outVideoCodecCtx->bit_rate = 120000;// 貌似会影响清晰度 videoCodecContext->bit_rate;
				...
				//打开编码器
				if ((ret = avcodec_open2(outVideoCodecCtx, outVideoCodec, NULL)) < 0)
				{
		...
		struct SwsContext* swsContext = sws_getContext(
			videoCodecContext->width, videoCodecContext->height, videoCodecContext->pix_fmt,//源格式
		...
		av_new_packet(pkt, videoCodecContext->width * videoCodecContext->height); //调整packet的数据
		...
		while (1)
		{
			...
			if (pkt->stream_index == videoIndex)
			{
				...
				while (ret >= 0)
				{
					...
					while (ret >= 0)
					{
						ret = avcodec_receive_packet(outVideoCodecCtx, pkt);
						if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
						{
							break;
						}
						...
						//Important:Delay
						//AVRational time_base = ofmtCtx/ifmtCtx->streams[videoIndex]->time_base;
...
int32_t RtmpPushDesktop()
{
	...
	do
	{
		...
		//输出编码
		//推送rtmp流到服务器  
		const char* outFilename = R"(rtmp://127.0.0.1:1935/live/desktop)";
		const char* ofmtName = "flv";//输出格式;
		...
		for (int i = 0; i < ifmtCtx->nb_streams; ++i)
		{
			...
			{
				// 调整输出流的一些属性
				outStream->time_base = AVRational{ 1,30 };//inStream->time_base;
				...
				outVideoCodecCtx->codec_id = ofmtCtx->oformat->video_codec;
				outVideoCodecCtx->codec_type = AVMEDIA_TYPE_VIDEO;
				outVideoCodecCtx->pix_fmt = AV_PIX_FMT_YUV420P;
				outVideoCodecCtx->width = videoCodecContext->width;
				outVideoCodecCtx->height = videoCodecContext->height;
